...
This folder contains scripts for creating thesis figures that are not generated by the other scripts in this repository.

`01_administrative_regions.ipynb`

Creates a overview map of the seven Bavarian administrative regions.


`02_New_Buildings_Vs_Existing_Stock.ipynb`

Visualizes the spatial distribution of newly constructed residential buildings relative to the existing building stock.

`03_Location_NHDAs.ipynb`

Creates an overview map showing the location of all detected New Housing Development Areas (NHDAs) across Bavaria.

`04_Distribution_CLC.ipynb`

Visualizes the distribution of the dominant pre-development CORINE Land Cover (CLC) and surrounding CLC classes of the detected NHDAs.

`05_Construction_Start_per_year.ipynb`

Maps the distribution of NHDA for each year.

`06_Development_Characteristics_Map.ipynb`

Creates district-level maps summarizing key development characteristics of the detected NHDAs.

`07_Res_Building_Type_Share_LK.ipynb`

Visualizes the dominant residential building types of NHDAs and reference areas at the district level.

`08_Annual_Env_Con_example.ipynb`
